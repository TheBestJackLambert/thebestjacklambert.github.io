...
const POPULAR_VIDEO_ID = "Bp5GJNn_ttE"; // Caro–Kann video (your most-viewed)
const RECENT_VIDEO_ID  = "Yfrf65VV9iY"; // Philidor video (use as “Recent” or swap later)

// Cheat-card “Watch breakdown” mapping
const WATCH_MAP = {
  traxler: { videoId: "i9v_zZA4VmI", ts: 0 }, // “CONQUER The Fried Liver Attack” (Traxler)
  scotch:  { videoId: "StP6jlUpdiA", ts: 0 }, // “Kasparov’s favorite opening” (Scotch Gambit)
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (canvas) {
    ...
    const step=()=>{
      ...
      for(const p of pts){
        ...
        ctx.beginPath(); ctx.arc(p.x,p.y,p.r,0,Math.PI*2); ctx.fillStyle='#ffd700'; ctx.fill();
      }
  ...
  if (cursor) {
    ...
    document.querySelectorAll('a,button,iframe,.tab').forEach(el=>{
  ...
  window.addEventListener('scroll',()=>{
    ...
    const bar=document.getElementById('scroll-progress'); if (bar) bar.style.width=pct+'%';
    if(backToTop){ if(window.scrollY>320) backToTop.classList.add('show'); else backToTop.classList.remove('show'); }
  });
  const io=new IntersectionObserver(es=>es.forEach(e=>{if(e.isIntersecting)e.target.classList.add('revealed');}),{threshold:.12});
  ...
  /* Stats (manual edit here) */
  const setText = (id, v) => { const el = document.getElementById(id); if (el) el.textContent = v; };
  setText('stat-subs',  '807');
  setText('stat-videos','6');
  setText('stat-elo',   '939');
  setText('stat-best',  '54,820');
